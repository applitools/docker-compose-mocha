...
module.exports = () => {
  ...
  async function copyConfigurations(srcBucket, destBucket) {
    ...
    const configs = await Promise.all(files.map(async (file) => file.download()));
  ...
  async function setupGcsBuckets(Storage) {
    ...
    if (!isConfigsBucketExists) {
      throw new Error('First create the bucket in env var TEST_CONFIGS_BUCKET');
    }
    ...
    if (!isLocksBucketExists) {
      throw new Error('First create the bucket in env var TEST_LOCKS_BUCKET');
    }
