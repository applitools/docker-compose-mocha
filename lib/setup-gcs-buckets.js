...
module.exports = () => {
  ...
  async function copyConfigurations(srcBucket, destBucket) {
    ...
    for (const file of files) {
      ...
      // await file.copy(destBucket, {metadata: {podConfHash: file.metadata.metadata.podConfHash}})
    }
  }
  ...
  async function setupGcsBuckets() {
    ...
    configsBucket = await gcsStorage.bucket(process.env.CONFIGS_BUCKET)
    locksBucket = await gcsStorage.bucket(process.env.LOCKS_BUCKET)

    await cleanBucket(configsBucket)
    await copyConfigurations(gcsStorage.bucket('wus-latest-configurations'), configsBucket)
