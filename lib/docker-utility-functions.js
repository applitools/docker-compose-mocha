...
async function cleanupContainersByEnvironmentName(envName/* : string */,
  ...
  await exec(`docker-compose -p ${envName} -f "${pathToComposeFile}" ${brutallyKill ? 'kill' : 'down'}`, { shell: '/bin/zsh' });
  ...
  await exec(`docker-compose -p ${envName} -f "${pathToComposeFile}" down -v`, { shell: '/bin/zsh' });
  if (!process.env.NOSPIN) {
    spinner2.stop();
